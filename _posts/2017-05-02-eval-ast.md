...
本文逐步分析，以实现一个加减乘除的计算器，并加一些为简单的特性，实现了之后，更复杂的语法树的求值也是类似的。
本文采用golang实现。
...
func (an *IntegerNode) Eval() Result {
    return Result{
        Typ:   TypeInteger,
        Value: an.intValue,
    }
...
func (an *AddNode) Eval() Result {
    //如果left是IntegerNode直接得到结果
    //如果
...
func (n *IntegerNode) Eval() Result {
    return Result{
        Typ:   TypeInteger,
        Value: n.val,
    }
